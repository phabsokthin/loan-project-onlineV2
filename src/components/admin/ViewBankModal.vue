        ...
        <div x-data="{ showPrivacyPolicy: true }">
            ...
            <form @submit.prevent="handleUpdatecodeWithdraw" x-show="showPrivacyPolicy" class="fixed inset-0 z-10 flex items-center justify-center">
                ...
                <div class="relative w-full max-w-screen-md m-4 overflow-hidden bg-white rounded-lg shadow-xl"
                    ...
                    <!-- Modal panel -->
                    <div class="px-6 py-4">
                        <h3 class="text-lg font-medium leading-6 text-gray-900">Modify Bank Account </h3>
                    </div>
                    <div class="max-w-screen-md p-6 space-y-3 overflow-y-auto prose"
                      ...
                      <input required type="text" class="w-full p-2 border rounded-md" v-model="bankName" readonly>
                      
                      
                        </div>
                         <div>
                             <label for="">ID Number</label>
                      <input required type="text" class="w-full p-2 border rounded-md" v-model="idNumber" readonly>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        onMounted(() => {
            if (props?.creditData) {
                bankName.value = props?.creditData?.bankName || "";
                idNumber.value = props?.creditData?.idNumber || "";
            }
        ...
        return { handleClose, codeWithdraw, bankName, idNumber}
    }
}
