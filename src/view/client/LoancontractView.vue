<template>
  <div class="min-h-screen py-3 back_image">
    ...
    <div class="w-full max-w-2xl px-4 mx-auto mt-10" v-motion-fade>
      ...
      <!-- Card -->
      <div class="w-full max-w-2xl p-6 mt-5 text-white shadow-lg bg-black/50 rounded-xl">
        <!-- Form -->
        <form v-if="!userDoc?.assigned_image" @submit.prevent="agreeSignature" class="">
          <!-- Left side -->
          <div class="w-full space-y-6">
            <h2 class="flex items-center space-x-3 text-white">
            ...
            <div>
              <input required v-model="signature_name" type="text" placeholder="Signature: *"
                class="w-full p-2 text-white bg-transparent border-b border-gray-300 focus:outline-none focus:border-yellow-400" />
            </div>

            <div>
              <canvas ref="signaturePad" class="w-full h-40 bg-white border border-gray-300 rounded-md"></canvas>
            </div>

            <!-- Buttons -->
            <div class="flex gap-2">
              <button type="button" @click="clearSignature"
                class="flex-1 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">
                Reset
              </button>
              <button v-if="!isLoanding" type="submit"
                class="flex-1 py-2 text-black bg-yellow-500 rounded-md hover:bg-yellow-600">
                Submit
              </button>
              <button v-else disabled class="flex-1 py-2 text-gray-200 bg-gray-600 rounded-md cursor-not-allowed">
                Please wait...
              </button>
            </div>
          </div>
